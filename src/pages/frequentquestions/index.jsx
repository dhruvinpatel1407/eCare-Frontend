...
const FrequentQuestions = () => {
  return (
    <div className="pt-20 max-w-2xl mx-auto px-4">
      <h2 className="text-3xl font-bold text-center mb-8 text-[#2D336B]">
      ...
      <Accordion allowMultiple>
        {faqItems.map((item, index) => (
          <AccordionItem
            key={index}
            header={({ state }) => (
              <div className="flex justify-between items-center py-4 text-lg font-semibold text-[#2D336B] cursor-pointer">
                {item.question}
                <FiChevronDown
                  className={`transform transition-transform duration-300 ml-6 text-[#7886C7] ${
                    state?.isEnter ? "rotate-180" : ""
                  }`}
                />
              </div>
            )}
            className={`border rounded-xl mb-4 px-6`}
            contentProps={{
              className:
                "pb-4 text-base text-[#2D336B] transition-all duration-300",
            }}
          >
            {item.answer}
          </AccordionItem>
